    ...
    <header className="bg-white dark:bg-gray-900">
      ...
      {menuOpen && (
        <nav className="md:hidden bg-white dark:bg-gray-800 p-4 space-y-2">
          ...
          <div className="mt-4">
            {isAuthenticated ? (
              <button className="bg-red-500 text-white w-full py-2 rounded-md">Logga ut</button>
            ) : (
              <>
                <Link href="/login" className="block text-gray-700 dark:text-gray-300 hover:text-blue-500">
                  Logga in
                </Link>
                <Link href="/register" className="block bg-blue-500 text-white text-center py-2 rounded-md mt-2">
                  Registrera dig
                </Link>
              </>
            )}
